...
export const TOOL_ADOPTION_QUESTIONS = [
  { id: 'materiality', kind: 'risk-criteria',
    text: "If this AI tool's output were incorrect, what would be the resulting real-world impact?",
    options: [
      { v: 0, label: 'Low stakes: an incorrect output constitutes a minor inconvenience (e.g., a poorly worded internal draft)' },
      { v: 1, label: 'Moderate: an incorrect output would require correction and could cause some rework or minor customer friction' },
      { v: 2, label: 'High: an incorrect output could cause material financial, legal, or reputational harm before being identified' },
      { v: 3, label: "Severe: an incorrect output could cause serious harm to an individual's safety, finances, legal standing, or access to a service" }
    ]},
  { id: 'autonomy', kind: 'risk-criteria',
    text: "To what extent does a person review this tool's output before it takes effect?",
    options: [
      ...
      { v: 2, label: 'A person spot-checks some outputs, but most proceed without review' },
  ...
  { id: 'dataSensitivity', kind: 'risk-criteria',
    ...
    options: [
      ...
      { v: 3, label: 'Sensitive regulated data, including health, financial, biometric, or data concerning children' }
    ]},
  { id: 'consequentialDecisions', kind: 'risk-criteria',
    ...
    options: [
      { v: 0, label: 'No; it does not factor into decisions concerning individual people' },
      { v: 1, label: 'It provides background information a person then uses to decide' },
      { v: 2, label: "It produces a recommendation that is typically followed with limited scrutiny" },
  ...
  { id: 'populationAffected', kind: 'risk-criteria',
    text: "How many individuals does this tool's use affect, and how vulnerable is that population?",
    options: [
      ...
      { v: 3, label: 'A large population that includes individuals less able to advocate for themselves (children, individuals with disabilities, individuals in crisis, applicants for essential services)' }
    ]},
  { id: 'reversibility', kind: 'risk-criteria',
    text: 'If this tool produces an adverse outcome, how difficult is it to reverse?',
    options: [
      { v: 0, label: 'Easily reversible, with no lasting effect (e.g., the task can simply be redone)' },
      { v: 1, label: 'Correctable: requires some effort, but a full remedy is realistic' },
      { v: 2, label: 'Difficult: material effort or cost is required to unwind, and some harm may have already occurred' },
      { v: 3, label: "Irreversible: once it occurs, it cannot meaningfully be undone (a decision was acted upon, information was already disclosed, or an individual already experienced the harm)" }
    ]},
  { id: 'thirdPartyDependency', kind: 'risk-criteria',
    text: "What degree of visibility or control does your organization have over how this tool operates?",
    options: [
      { v: 0, label: 'Fully in-house, or a vendor tool whose behavior and data handling the organization can fully inspect and control' },
      { v: 1, label: 'A vendor tool with a documented, reviewed data-processing agreement and reasonable transparency' },
      { v: 2, label: 'A vendor tool relied upon without a reviewed agreement, or with limited visibility into how it processes organizational data' },
      { v: 3, label: 'A black-box third-party or subcontracted-model dependency, with no meaningful visibility into its operation or the destination of organizational data' }
    ]},
  { id: 'ma-adopt-1', kind: 'decision', fn: 'manage', nistRef: 'NIST AI RMF 1.0 (Jan 2023), Core, MANAGE 1.1',
    text: 'Has a documented go/no-go determination been made regarding whether this AI tool achieves its intended purpose, prior to deployment?',
    hint: "NIST AI RMF MANAGE 1.1: a documented determination of whether the system's development or deployment should proceed, as distinct from simply commencing use.",
    options: [
      { v: 0, label: 'No determination was made; the tool is already in use, or about to be, without evaluation of whether it meets the intended need' },
      { v: 1, label: 'An informal decision was made (a discussion or a brief trial), but it was not documented and no specific individual owns it' },
      { v: 2, label: 'A decision was made and documented, but it was not reviewed by anyone beyond the individual proposing the tool' },
      { v: 3, label: 'A documented go/no-go determination was made, reviewed by an individual other than the requester, based on whether the tool achieves its intended purpose' }
    ]},
  { id: 'ma-adopt-2', kind: 'decision', fn: 'manage', nistRef: 'NIST AI RMF 1.0 (Jan 2023), Core, MANAGE 2.1',
    text: 'Prior to adopting this tool, were its resource requirements and risk weighed against realistic alternatives, including a non-AI approach?',
    hint: 'NIST AI RMF MANAGE 2.1: consideration of viable non-AI alternatives is part of this determination, not a subsequent addition.',
    options: [
      { v: 0, label: 'No comparison was made; the tool was adopted without evaluation against any alternative' },
...
export const BASE_QUESTIONS = [
  { id: 'g1', fn: 'govern', module: 'base', dimension: 'controls-evidence', visibilityTag: 'strategic', depths: ['quick','standard','comprehensive'],
    text: 'Does your organization maintain a written policy governing employee use of AI tools?',
    hint: 'Includes ChatGPT, Copilot, Gemini, and any AI feature embedded within existing software.',
    options: [
  ...
  { id: 'g2', fn: 'govern', module: 'base', dimension: 'ownership-accountability', visibilityTag: 'strategic', depths: ['quick','standard','comprehensive'],
    text: 'Who is accountable when an AI tool gives rise to an adverse outcome?',
    hint: 'For example, erroneous output delivered to a customer, or sensitive data exposed to a model.',
    options: [
      { v: 0, label: 'No one has been named' },
      { v: 1, label: 'Unclear, dependent on the specific circumstances' },
  ...
  { id: 'g3', fn: 'govern', module: 'base', dimension: 'controls-evidence', visibilityTag: 'operational', depths: ['standard','comprehensive'],
    ...
    options: [
      ...
      { v: 1, label: 'Ad hoc, and reviewed on an inconsistent basis' },
      { v: 2, label: 'An informal review is conducted' },
      { v: 3, label: 'A formal review process with defined approval criteria' }
    ]},
  { id: 'g4', fn: 'govern', module: 'base', dimension: 'controls-evidence', visibilityTag: 'operational', depths: ['comprehensive'],
    text: 'Does your organization provide training to employees on responsible AI use?',
    options: [
      { v: 0, label: 'No training' },
      { v: 1, label: 'Addressed once, during onboarding, with no follow-up' },
      { v: 2, label: 'Periodic refresher training is provided' },
      { v: 3, label: 'Structured, role-specific training, reviewed and updated on a regular basis' }
    ]},
  { id: 'g5', fn: 'govern', module: 'base', dimension: 'ownership-accountability', visibilityTag: 'strategic', depths: ['comprehensive'],
    text: 'Are AI-related risks discussed at the leadership or board level?',
    options: [
      { v: 0, label: 'Never' },
      { v: 1, label: 'Only after incidents' },
      { v: 2, label: 'Discussed occasionally as part of broader technology updates' },
      { v: 3, label: 'Discussed on a regular basis, with dedicated time on the agenda' }
    ]},
  { id: 'm1', fn: 'map', module: 'base', dimension: 'inventory-visibility', visibilityTag: 'operational', depths: ['quick','standard','comprehensive'],
    text: 'Does your organization have visibility into which AI tools are actively in use by the team?',
    hint: 'Includes AI functionality embedded within products already licensed by the organization.',
    options: [
      { v: 0, label: 'No visibility' },
      { v: 1, label: 'A general awareness exists, but it is not documented' },
      { v: 2, label: 'A documented list exists and is updated on an occasional basis' },
      { v: 3, label: 'A current inventory is maintained and reviewed on a quarterly basis' }
    ]},
  { id: 'm2', fn: 'map', module: 'base', dimension: 'risk-classification', visibilityTag: 'operational', depths: ['quick','standard','comprehensive'],
    text: 'For each AI use case, has your organization identified who could be harmed in the event of an error?',
    options: [
      { v: 0, label: 'Not considered' },
      { v: 1, label: 'Considered for one or two high-visibility use cases' },
      { v: 2, label: 'Considered for most cases' },
      { v: 3, label: 'A systematic impact analysis is conducted for each use case' }
    ]},
  { id: 'm3', fn: 'map', module: 'base', dimension: 'risk-classification', visibilityTag: 'strategic', depths: ['standard','comprehensive'],
    text: 'Does your organization classify AI use cases by risk level?',
    hint: 'For example: low risk (drafting), medium risk (customer response), high risk (hiring, credit, or medical decisions).',
    options: [
      { v: 0, label: 'No classification' },
      { v: 1, label: 'Assessed informally, on a case-by-case basis' },
      { v: 2, label: 'Simple risk tiers are applied on an inconsistent basis' },
      { v: 3, label: 'Documented risk tiers are applied to every new use case' }
    ]},
  { id: 'm4', fn: 'map', module: 'base', dimension: 'inventory-visibility', visibilityTag: 'technical-build', depths: ['comprehensive'],
    text: 'Does your organization know what data each AI tool is able to access?',
    options: [
      { v: 0, label: 'No, and there is no straightforward means of determining this' },
      { v: 1, label: "Known only for the organization's most significant tools" },
      { v: 2, label: 'Largely known, with some gaps' },
      { v: 3, label: 'Fully mapped and reviewed on a regular basis' }
    ]},
  { id: 'm5', fn: 'map', module: 'base', dimension: 'risk-classification', visibilityTag: 'legal-compliance', depths: ['comprehensive'],
    text: 'Does your organization evaluate third-party AI vendors prior to onboarding?',
    options: [
      { v: 0, label: 'No vendor review' },
      { v: 1, label: "An informal review of the vendor's website is conducted" },
      { v: 2, label: 'Basic security or privacy questionnaire' },
      { v: 3, label: 'A structured review is conducted, including AI-specific risk considerations' }
    ]},
  { id: 'me1', fn: 'measure', module: 'base', dimension: 'monitoring-response', visibilityTag: 'operational', depths: ['quick','standard','comprehensive'],
    text: 'How does your organization verify that AI output is accurate and reliable?',
    options: [
      { v: 0, label: 'No verification is performed' },
      { v: 1, label: 'Users perform spot checks when an anomaly is observed' },
      { v: 2, label: 'Sample-based reviews are performed on high-stakes outputs' },
      { v: 3, label: 'A structured review process is in place, supported by documented metrics' }
    ]},
  { id: 'me2', fn: 'measure', module: 'base', dimension: 'monitoring-response', visibilityTag: 'operational', depths: ['quick','standard','comprehensive'],
    text: 'Does your organization track incidents related to AI use?',
    hint: 'For example, erroneous outputs sent externally, prompt leakage, or biased results.',
    options: [
      { v: 0, label: 'No tracking' },
      { v: 1, label: 'Only major incidents are discussed' },
      { v: 2, label: 'An informal log is maintained' },
      { v: 3, label: 'A formal incident register is maintained, with a defined review cadence' }
    ]},
  { id: 'me3', fn: 'measure', module: 'base', dimension: 'monitoring-response', visibilityTag: 'technical-build', depths: ['standard','comprehensive'],
    text: 'Does your organization monitor AI outputs that affect individuals for evidence of bias?',
    hint: 'Particularly relevant to hiring, customer service, pricing, and eligibility decisions.',
    options: [
      { v: 0, label: 'Not monitored' },
      { v: 1, label: 'Acknowledged as a risk, but no active checks are performed' },
      { v: 2, label: 'Periodic manual review is conducted' },
      { v: 3, label: 'Regular, structured testing is conducted' }
    ]},
  { id: 'me4', fn: 'measure', module: 'base', dimension: 'controls-evidence', visibilityTag: 'legal-compliance', depths: ['comprehensive'],
    text: 'Does your organization maintain records of AI-driven decisions that affect customers or employees?',
    options: [
      { v: 0, label: 'No records' },
      { v: 1, label: 'Records are retained only in response to a complaint' },
      { v: 2, label: 'Some records are maintained, on an inconsistent basis' },
      { v: 3, label: 'Systematic logging is in place, governed by a defined retention policy' }
    ]},
  { id: 'me5', fn: 'measure', module: 'base', dimension: 'monitoring-response', visibilityTag: 'operational', depths: ['comprehensive'],
    text: 'How frequently does your organization re-evaluate the AI tools in use?',
    options: [
      { v: 0, label: 'Never after initial adoption' },
      { v: 1, label: 'Only in response to an incident' },
      { v: 2, label: 'Annually' },
      { v: 3, label: 'On a defined cadence, against established criteria' }
    ]},
  { id: 'ma1', fn: 'manage', module: 'base', dimension: 'monitoring-response', visibilityTag: 'legal-compliance', depths: ['quick','standard','comprehensive'],
    text: 'In the event that an AI tool were to produce a harmful output today, what is the defined next step?',
    options: [
      { v: 0, label: 'No formal response; the outcome depends on who identifies the issue' },
      { v: 1, label: 'The issue would be raised informally' },
      { v: 2, label: 'An escalation path exists but has not been tested' },
      { v: 3, label: 'A documented response plan exists and has been tested at least once' }
    ]},
  { id: 'ma2', fn: 'manage', module: 'base', dimension: 'monitoring-response', visibilityTag: 'technical-build', depths: ['quick','standard','comprehensive'],
    text: 'Can your organization disable or roll back an AI tool on short notice, if required?',
    options: [
      { v: 0, label: 'No; this would require several days or longer' },
      { v: 1, label: 'Yes, though the process is slow and resource-intensive' },
      { v: 2, label: 'Yes, within several hours' },
      { v: 3, label: 'Yes, on short notice, supported by a documented process' }
    ]},
  { id: 'ma3', fn: 'manage', module: 'base', dimension: 'controls-evidence', visibilityTag: 'legal-compliance', depths: ['standard','comprehensive'],
    text: 'Are customers or employees made aware when they are interacting with an AI system?',
    options: [
      { v: 0, label: 'Never disclosed' },
      { v: 1, label: 'Disclosed inconsistently' },
      { v: 2, label: 'Disclosed in most customer-facing cases' },
      { v: 3, label: 'Disclosed by default, in accordance with policy' }
    ]},
  { id: 'ma4', fn: 'manage', module: 'base', dimension: 'controls-evidence', visibilityTag: 'legal-compliance', depths: ['comprehensive'],
    text: 'Does your organization provide a mechanism for individuals to challenge or appeal an AI-driven decision?',
    options: [
  ...
  { id: 'ma5', fn: 'manage', module: 'base', dimension: 'monitoring-response', visibilityTag: 'operational', depths: ['comprehensive'],
    ...
    options: [
      { v: 0, label: 'No review is triggered' },
      { v: 1, label: 'Only after incidents' },
      { v: 2, label: 'Conducted on an inconsistent basis for major changes' },
      { v: 3, label: 'Conducted as standard practice for every significant change' }
    ]},
...
// stated scope -- see getQuestionsForAssessment in logic.js.
  { id: 'g6', fn: 'govern', module: 'base', dimension: 'risk-classification', visibilityTag: 'strategic', depths: ['standard','comprehensive'],
    text: "Does your organization apply differentiated levels of scrutiny to AI tools based on risk, or is the same level of review applied uniformly?",
    options: [
      { v: 0, label: "Every AI tool receives the same level of review, or no tools are reviewed at all" },
      { v: 1, label: "An informal sense exists that certain tools warrant closer attention, without documented criteria" },
      { v: 2, label: "A basic tiering structure exists (e.g., low, medium, high) but is not consistently applied" },
      { v: 3, label: "A documented risk-tolerance policy determines review intensity and is applied consistently" }
    ]},
  { id: 'g7', fn: 'govern', module: 'base', dimension: 'controls-evidence', visibilityTag: 'operational', depths: ['comprehensive'],
    text: "When an AI tool is decommissioned, does your organization follow a defined process for retiring it safely?",
    hint: "Includes revoking access, migrating or deleting data held by the tool, and documenting the rationale for retirement.",
    options: [
      { v: 0, label: "No formal process; tools are simply discontinued with no further action taken" },
      { v: 1, label: "Access is generally revoked over time, but the process is not documented" },
      { v: 2, label: "A basic checklist exists, covering access and data, but is not consistently followed" },
      { v: 3, label: "A documented offboarding process is followed consistently, including data handling and a recorded rationale for retirement" }
    ]},
  { id: 'g8', fn: 'govern', module: 'base', dimension: 'ownership-accountability', visibilityTag: 'strategic', depths: ['standard','comprehensive'],
    text: "When decisions are made regarding which AI tools to adopt or how to govern them, which stakeholders are involved?",
    hint: "NIST's framing addresses diversity of discipline and role as much as demographics: the question extends beyond who provides sign-off to who is genuinely consulted.",
    options: [
      { v: 0, label: "A single individual decides, typically whoever advocated for the tool" },
      { v: 1, label: "IT or leadership decides without input from the intended end users" },
      { v: 2, label: "Input is gathered informally from a small number of relevant stakeholders prior to the decision" },
      { v: 3, label: "A defined cross-functional group (e.g., legal, operations, the affected department, IT/security) reviews the decision prior to adoption" }
    ]},
  { id: 'g9', fn: 'govern', module: 'base', dimension: 'ownership-accountability', visibilityTag: 'operational', depths: ['standard','comprehensive'],
    text: "Is it clearly defined who is responsible for reviewing AI-generated work prior to use, as distinct from what the AI is permitted to perform unsupervised?",
    options: [
      { v: 0, label: "Not defined; individuals exercise their own judgment on a case-by-case basis" },
      { v: 1, label: "A general expectation exists (e.g., always verify AI output) but nothing is specified per tool" },
      { v: 2, label: "Defined for the highest-visibility use cases only" },
      { v: 3, label: "Documented per AI use case, specifying what is human-reviewed, what operates autonomously, and who is responsible" }
    ]},
  { id: 'g10', fn: 'govern', module: 'base', dimension: 'monitoring-response', visibilityTag: 'operational', depths: ['comprehensive'],
    text: "If an employee raises a concern regarding the misuse of an AI tool or a risky output, what is the defined response?",
    options: [
      { v: 0, label: "No clear channel exists; the employee would need to determine independently who to notify" },
      { v: 1, label: "The employee could raise it with their manager, though there is no expectation it would be escalated further" },
      { v: 2, label: "An informal understanding exists that concerns are escalated, though this is not documented" },
      { v: 3, label: "A documented channel exists, and employees are assured that raising a concern will not be held against them" }
    ]},
  { id: 'g11', fn: 'govern', module: 'base', dimension: 'monitoring-response', visibilityTag: 'operational', depths: ['comprehensive'],
    text: "Do customers, clients, or members of the public have a defined mechanism to flag a problem with an AI-driven interaction?",
    options: [
      { v: 0, label: "No dedicated mechanism exists" },
      { v: 1, label: "General customer support exists, but AI-specific issues are not tracked separately" },
      { v: 2, label: "Feedback is collected but is not routed for review" },
      { v: 3, label: "A defined channel exists, and flagged issues are reviewed by an individual responsible for AI governance" }
    ]},
  { id: 'g12', fn: 'govern', module: 'base', dimension: 'monitoring-response', visibilityTag: 'strategic', depths: ['comprehensive'],
    text: "When external feedback identifies a genuine problem with an AI tool, does it result in a substantive change?",
    options: [
      { v: 0, label: "No process connects feedback to any change" },
      { v: 1, label: "It may prompt a discussion, but rarely results in a documented change" },
      { v: 2, label: "Significant issues occasionally lead to a policy or configuration change" },
  ...
  { id: 'g13', fn: 'govern', module: 'base', dimension: 'controls-evidence', visibilityTag: 'legal-compliance', depths: ['comprehensive'],
    text: "In the event that a vendor's AI tool were to fail, be breached, or be found to be seriously flawed, does your organization have a defined contingency plan?",
    hint: "Distinct from evaluating a vendor prior to onboarding (addressed separately): this concerns the period after the tool is already in use.",
    options: [
      { v: 0, label: "No plan exists; the response would be determined if and when the event occurred" },
      { v: 1, label: "The organization would likely discontinue use of the tool, though no documented process exists" },
      { v: 2, label: "An informal understanding exists of which stakeholders would be involved, though this is not documented" },
      { v: 3, label: "A documented contingency plan exists, including an assessment of how quickly the organization could operate without the tool" }
    ]},
  { id: 'm6', fn: 'map', module: 'base', dimension: 'inventory-visibility', visibilityTag: 'operational', depths: ['standard','comprehensive'],
    text: "Prior to adopting an AI tool, does your organization consult the intended day-to-day users regarding their requirements and anticipated use?",
    options: [
      { v: 0, label: "No; tools are selected and deployed without prior consultation of end users" },
      { v: 1, label: "Occasionally, and only on an informal basis" },
      { v: 2, label: "End users are generally consulted, but their input is not documented or systematically considered" },
  ...
  { id: 'm7', fn: 'map', module: 'base', dimension: 'ownership-accountability', visibilityTag: 'strategic', depths: ['standard','comprehensive'],
    text: "Does your organization maintain a clear, documented rationale for its use of AI, articulating its intended purpose in relation to organizational mission and goals?",
    options: [
      { v: 0, label: "No; AI tools are adopted individually, without a stated purpose" },
      { v: 1, label: "A general sense exists that AI adoption is worthwhile, but this has not been documented" },
      { v: 2, label: "Goals exist for specific initiatives, but no overarching statement has been documented" },
  ...
  { id: 'm8', fn: 'map', module: 'base', dimension: 'monitoring-response', visibilityTag: 'strategic', depths: ['comprehensive'],
    text: "For AI tools currently in use, has your organization determined whether they are delivering the anticipated value?",
    hint: "Distinct from verifying output accuracy: this concerns whether the tool justifies its cost, rather than whether its outputs are correct.",
    options: [
      { v: 0, label: "No; tools are adopted with no subsequent follow-up" },
      { v: 1, label: "A general impression exists, though no formal measurement has been conducted" },
  ...
  { id: 'm9', fn: 'map', module: 'base', dimension: 'risk-classification', visibilityTag: 'strategic', depths: ['standard','comprehensive'],
    text: "Has your organization explicitly determined and documented the level of AI risk it is willing to accept?",
    hint: "For example: AI will not be used to make a final hiring decision without human sign-off.",
    options: [
      { v: 0, label: "No; risk tolerance has not been discussed" },
      { v: 1, label: "Leadership holds informal views on the matter, though these have not been documented" },
  ...
  { id: 'm10', fn: 'map', module: 'base', dimension: 'inventory-visibility', visibilityTag: 'technical-build', depths: ['standard','comprehensive'],
    text: "For each AI tool in use, has your organization determined what type of AI it employs, for example, whether it generates content, classifies or scores information, or produces recommendations?",
    hint: "This distinction is material, as a tool that scores or classifies individuals carries a different risk profile than one that drafts text.",
    options: [
      { v: 0, label: "No; tools are regarded generically as an AI feature, with no further classification" },
      { v: 1, label: "Understood in general terms, and only for the most prominent tools" },
      { v: 2, label: "Understood for most tools, though this has not been documented" },
  ...
  { id: 'm11', fn: 'map', module: 'base', dimension: 'inventory-visibility', visibilityTag: 'technical-build', depths: ['comprehensive'],
    text: "Has your organization identified the specific situations in which an AI tool's output should not be relied upon without prior human review?",
    options: [
      { v: 0, label: "No; the tool is presumed to be generally reliable" },
      { v: 1, label: "A general awareness exists that AI can produce incorrect output, though nothing specific has been identified" },
      { v: 2, label: "Known for the most evident failure cases, though this has not been documented" },
  ...
  { id: 'm12', fn: 'map', module: 'base', dimension: 'controls-evidence', visibilityTag: 'operational', depths: ['standard','comprehensive'],
    text: "Prior to adopting an AI tool, does your organization document the specific benefit it is expected to provide?",
    options: [
      { v: 0, label: "No; tools are adopted on the basis of perceived usefulness, with no documentation" },
      { v: 1, label: "An informal rationale exists but has not been documented" },
      { v: 2, label: "Documented for major purchases only, and not for smaller or no-cost tools" },
      { v: 3, label: "Documented for every tool prior to adoption, including the criteria for determining success" }
    ]},
  { id: 'm13', fn: 'map', module: 'base', dimension: 'risk-classification', visibilityTag: 'operational', depths: ['standard','comprehensive'],
    text: "Prior to adopting an AI tool, does your organization document the potential downsides, including cost and risk, in addition to the anticipated benefits?",
    options: [
      { v: 0, label: "No; only the anticipated benefit is considered" },
      { v: 1, label: "Potential downsides are discussed informally but not documented" },
  ...
  { id: 'm14', fn: 'map', module: 'base', dimension: 'controls-evidence', visibilityTag: 'operational', depths: ['comprehensive'],
    text: "Is there a documented boundary defining the approved uses of an AI tool, as distinct from uses that have not been formally authorized?",
    hint: "For example: approved for drafting internal memoranda, but not for external communications.",
    options: [
      { v: 0, label: "No defined boundary; tools are used at individual discretion" },
      { v: 1, label: "An informal, undocumented sense of appropriate use exists" },
  ...
  { id: 'm15', fn: 'map', module: 'base', dimension: 'controls-evidence', visibilityTag: 'operational', depths: ['comprehensive'],
    text: "Do users of a given AI tool possess sufficient competence to use it effectively, including its known limitations, failure modes, and how to obtain reliable results?",
    hint: "Distinct from general AI-use training: this concerns tool-specific competence rather than general awareness.",
    options: [
      { v: 0, label: "No; the tool is provided with no tool-specific guidance" },
      { v: 1, label: "Users develop competence independently over time" },
      { v: 2, label: "Informal guidance is shared among users, with no structured approach" },
  ...
  { id: 'm16', fn: 'map', module: 'base', dimension: 'ownership-accountability', visibilityTag: 'operational', depths: ['standard','comprehensive'],
    ...
    options: [
      ...
      { v: 1, label: "A general expectation exists that output should be reviewed, without specifying by whom" },
  ...
  { id: 'm17', fn: 'map', module: 'base', dimension: 'controls-evidence', visibilityTag: 'technical-build', depths: ['comprehensive'],
    text: "For AI tools composed of multiple components, for example, an internal workflow combined with a vendor's model and a plugin or integration, has your organization identified where the applicable risk controls reside?",
    ...
    options: [
      { v: 0, label: "No; it is assumed the vendor addresses this, and it has not been verified" },
      { v: 1, label: "A general assumption exists, never verified" },
      { v: 2, label: "Verified for major tools, not for smaller integrations" },
      { v: 3, label: "Documented per tool, specifying which controls fall under vendor responsibility and which fall under the organization's own responsibility" }
    ]},
  { id: 'm18', fn: 'map', module: 'base', dimension: 'monitoring-response', visibilityTag: 'operational', depths: ['comprehensive'],
    text: "Beyond an initial review at adoption, does your organization periodically reassess whether an AI tool continues to have the anticipated impact, positive or negative?",
    hint: "Distinct from the external-feedback questions above: this refers to a scheduled internal review, rather than a reactive response to external complaints.",
    options: [
      { v: 0, label: "No; impact is assessed once, if at all, and is not subsequently revisited" },
  ...
  { id: 'me6', fn: 'measure', module: 'base', dimension: 'monitoring-response', visibilityTag: 'technical-build', depths: ['comprehensive'],
    text: "When assessing whether an AI tool is performing as expected, does your organization prioritize the most material risks, or whatever is most readily measurable?",
    hint: "Distinct from the existing me1 (general output-quality checks): this concerns whether the assessment is prioritized toward the tool's most significant risks, rather than any convenient measure of quality.",
    options: [
      { v: 0, label: "Nothing is specifically measured: no metrics defined" },
      { v: 1, label: "Whatever is most readily measurable is assessed, not necessarily the highest-risk aspects" },
      { v: 2, label: "The organization has identified the most material risks but has not developed specific metrics for them" },
  ...
  { id: 'me7', fn: 'measure', module: 'base', dimension: 'controls-evidence', visibilityTag: 'operational', depths: ['comprehensive'],
    text: "Who verifies that an AI tool is operating safely and as intended: the individual who implemented it, or a party independent of that decision?",
    hint: "Independence does not require an external auditor at small-to-mid-size business scale; an internal party not involved in selecting or configuring the tool is sufficient.",
    options: [
      { v: 0, label: "Only the individual who implemented or advocates for the tool conducts the review" },
      { v: 1, label: "Another party occasionally reviews it, on an informal basis" },
  ...
  { id: 'me8', fn: 'measure', module: 'base', dimension: 'monitoring-response', visibilityTag: 'technical-build', depths: ['comprehensive'],
    text: "Is an AI tool's potential for causing harm, for example, through dangerous advice or an unsafe recommendation, assessed on an ongoing basis, or presumed acceptable following initial setup?",
    options: [
  ...
  { id: 'me9', fn: 'measure', module: 'base', dimension: 'monitoring-response', visibilityTag: 'technical-build', depths: ['comprehensive'],
    text: "For AI tools capable of taking action within organizational systems or handling content originating outside the organization, has your organization assessed the potential for manipulation into unintended actions?",
    hint: "This represents an active, current risk category; 'prompt injection' is the industry term. A useful test, drawn from current security industry guidance (OWASP, 2026): does the tool have (1) access to sensitive data, (2) exposure to content originating outside the organization, and (3) the ability to communicate or take action externally? A tool meeting all three criteria should require a human-approval step prior to acting; a tool meeting only one or two criteria presents comparatively lower risk.",
    options: [
      { v: 0, label: "Not assessed; the tool is used without regard to this consideration" },
  ...
  { id: 'me10', fn: 'measure', module: 'base', dimension: 'risk-classification', visibilityTag: 'operational', depths: ['comprehensive'],
    text: "Has your organization considered the environmental footprint of the AI tools it relies upon, including energy or compute consumption, at even a basic level?",
    hint: "Lower priority than the other measurement questions for most small-to-mid-size businesses deploying third-party tools rather than training proprietary models, though a growing number of vendors publish this data, and certain clients and funders have begun to request it.",
    options: [
      { v: 0, label: "Never considered" },
      { v: 1, label: "Recognized as a consideration, though not yet examined" },
      { v: 2, label: "Examined for a specific reason (e.g., at the request of a client or funder), but not tracked on an ongoing basis" },
  ...
  { id: 'me11', fn: 'measure', module: 'base', dimension: 'monitoring-response', visibilityTag: 'operational', depths: ['comprehensive'],
    text: "For AI use cases that lack a straightforward means of measuring whether something is going wrong, for example, subtle bias in written content, tone issues, or nuanced judgment calls, does your organization track those risks through an alternative method?",
    hint: "Certain risks do not lend themselves to a clear metric. The relevant question is whether any tracking approach exists for these risks, not whether a precise figure can be produced.",
    options: [
      { v: 0, label: "Risks that are difficult to measure are not tracked at all; only readily measurable items are assessed" },
      { v: 1, label: "Acknowledged informally, no tracking approach" },
      { v: 2, label: "Tracked for the highest-visibility case only" },
      { v: 3, label: "A defined approach exists for tracking risks without a clean metric, such as periodic spot checks or structured review" }
    ]},
  { id: 'me12', fn: 'measure', module: 'base', dimension: 'monitoring-response', visibilityTag: 'operational', depths: ['comprehensive'],
    text: "When determining what to assess regarding an AI tool's performance, do individuals with expertise in the specific use case, and not solely the tool itself, contribute to that determination?",
    hint: "Distinct from me13 below: this concerns the design of what is measured, prior to obtaining results. Item me13 concerns the subsequent validation of results.",
    options: [
      ...
      { v: 3, label: "What is measured is defined in consultation with individuals who understand the specific deployment context" }
    ]},
  { id: 'me13', fn: 'measure', module: 'base', dimension: 'monitoring-response', visibilityTag: 'operational', depths: ['comprehensive'],
    text: "After an AI tool's performance has been assessed, does an individual with relevant expertise confirm that the results accurately reflect actual performance, or do the figures stand without further validation?",
    hint: "Distinct from me12 above: that item concerns the design of the measurement itself; this item concerns the validation of its findings.",
    options: [
      { v: 0, label: "Results are not reviewed beyond the individual who performed the check" },
      ...
      { v: 3, label: "Results are routinely reviewed with individuals possessing relevant context, to confirm they reflect actual performance" }
    ]},
  { id: 'me14', fn: 'measure', module: 'base', dimension: 'monitoring-response', visibilityTag: 'operational', depths: ['comprehensive'],
    text: "Over time, can your organization identify specific evidence that an AI tool's performance has improved or declined, or does its assessment remain a general impression?",
    options: [
      { v: 0, label: "No tracking over time; only a general sense of continued usefulness" },
      { v: 1, label: "Noted informally when a clear failure occurs" },
  ...
  { id: 'ma6', fn: 'manage', module: 'base', dimension: 'risk-classification', visibilityTag: 'strategic', depths: ['comprehensive'],
    text: "When an AI tool is known to carry material risk, is the level of attention and resources allocated to managing it proportionate to the severity of that risk?",
    hint: "NIST's Playbook guidance addresses proportional resourcing specifically: higher-risk systems should receive greater oversight attention, and that allocation decision should be documented rather than assumed.",
    options: [
      { v: 0, label: "Resources and attention are not allocated by risk level; all tools receive a broadly similar level of attention, or none" },
      { v: 1, label: "Higher-risk tools informally receive greater attention, though this is neither documented nor applied consistently" },
  ...
  { id: 'ma7', fn: 'manage', module: 'base', dimension: 'controls-evidence', visibilityTag: 'legal-compliance', depths: ['comprehensive'],
    text: "After mitigating an AI tool's risk to the extent practicable, is the residual risk, the risk the organization is knowingly accepting, documented, and would affected parties be made aware of it?",
    ...
    options: [
      { v: 0, label: "Not tracked; once a mitigation is in place, the remaining risk is not further discussed" },
      { v: 1, label: "Understood informally by the individual who implemented the tool, but not documented" },
  ...
  { id: 'ma8', fn: 'manage', module: 'base', dimension: 'monitoring-response', visibilityTag: 'technical-build', depths: ['comprehensive'],
    text: "Beyond initial setup, does your organization maintain active mechanisms, such as monitoring, access controls, and a feedback channel, to sustain the safe performance of an AI tool as its use evolves?",
    hint: "Distinct from the existing m8 and m18: those items address whether the organization periodically reviews a tool's continued value or impact, a review-cadence question. This item addresses whether active technical and operational mechanisms, including monitoring, controls, and a feedback loop, exist on a day-to-day basis to sustain safe performance, rather than a scheduled review.",
    options: [
      { v: 0, label: "No ongoing mechanisms; the tool continues to operate as originally configured, without monitoring" },
      { v: 1, label: "Informal awareness exists if an issue arises, though no active mechanism is in place" },
...
export const NONPROFIT_QUESTIONS = [
  { id: 'np1', fn: 'govern', module: 'nonprofit', dimension: 'controls-evidence', depths: ['quick','standard','comprehensive'],
    text: 'Does your organization rely on free, personal-account AI tools for organizational work?',
    hint: 'Common among cost-constrained nonprofit organizations, though this practice introduces data risk in the absence of enterprise-grade controls.',
    options: [
      { v: 0, label: 'Yes, extensively, across the staff population' },
      { v: 1, label: 'Yes, on an informal basis, by certain staff members' },
      { v: 2, label: 'Predominantly enterprise accounts, with some personal accounts remaining in use' },
      { v: 3, label: 'Fully migrated to enterprise or organizational accounts' }
    ]},
  { id: 'np2', fn: 'govern', module: 'nonprofit', dimension: 'ownership-accountability', depths: ['standard','comprehensive'],
    text: 'Has the board of directors been briefed on AI adoption and associated risk?',
    hint: 'Board oversight represents a specific governance obligation for 501(c)(3) organizations.',
    options: [
      { v: 0, label: 'No board discussion has taken place' },
      { v: 1, label: 'Mentioned briefly, with no structured discussion' },
      { v: 2, label: 'Discussed on one occasion, with no ongoing agenda item' },
  ...
  { id: 'np3', fn: 'map', module: 'nonprofit', dimension: 'inventory-visibility', depths: ['standard','comprehensive'],
    text: 'Has your organization determined whether donor data has been entered into any AI tool?',
    hint: "Includes donor names combined with giving amounts, donor communications, and exports from Raiser's Edge, Salesforce, or similar platforms.",
    options: [
      { v: 0, label: 'No determination has been made, and no controls are in place' },
      { v: 1, label: 'Presumed likely, though the scope is unclear' },
      { v: 2, label: 'Limited, known use, with anonymization applied' },
      { v: 3, label: 'An explicit policy prohibits this practice, and compliance has been verified' }
    ]},
  { id: 'np4', fn: 'govern', module: 'nonprofit', dimension: 'controls-evidence', depths: ['comprehensive'],
    text: 'Has your organization applied for nonprofit-discounted enterprise AI accounts?',
    hint: 'For example, Google Workspace for Nonprofits, Canva for Nonprofits, or the ChatGPT Business nonprofit rate, which are often provided at no cost or at a substantial discount.',
    options: [
      { v: 0, label: 'Not aware that such programs exist' },
      { v: 1, label: 'Aware, but no application has been submitted' },
      { v: 2, label: 'Applied for one or two such programs' },
  ...
  { id: 'np5', fn: 'measure', module: 'nonprofit', dimension: 'inventory-visibility', depths: ['comprehensive'],
    text: "Where the organization's CRM platform, for example, Salesforce, Raiser's Edge, or Bloomerang, includes AI features, has your organization determined whether those features are active?",
    hint: 'Many nonprofit CRM platforms now include AI features, which may or may not be enabled by default.',
    options: [
      { v: 0, label: 'Uncertain what AI features are present' },
      { v: 1, label: 'Aware that certain features exist, though it is unclear which are enabled' },
      { v: 2, label: 'Reviewed on one occasion, with no ongoing check' },
...
export const YOUTH_QUESTIONS = [
  { id: 'y1', fn: 'govern', module: 'youth', dimension: 'inventory-visibility', depths: ['quick','standard','comprehensive'],
    text: 'Do any AI tools used by the team process data concerning individuals under the age of 13?',
    hint: 'COPPA applies to children under the age of 13. This includes names, program records, incident reports, and photographs.',
    options: [
      { v: 0, label: 'Yes, on personal-account AI tools, without controls in place' },
      { v: 1, label: 'Yes, unclear which controls are in place' },
      { v: 2, label: 'Yes, only on tools with compliant Data Processing Agreements' },
      { v: 3, label: 'No youth data is entered into any AI tool' }
    ]},
  { id: 'y2', fn: 'govern', module: 'youth', dimension: 'controls-evidence', depths: ['quick','standard','comprehensive'],
    text: 'Does your organization maintain a written policy specifically addressing AI use in youth programming?',
    hint: 'Distinct from the general staff AI policy, this addresses what AI may and may not be used for in connection with minors.',
    options: [
      { v: 0, label: 'No youth-specific AI policy exists' },
      { v: 1, label: 'Youth-related considerations are referenced only within the general AI policy' },
      { v: 2, label: 'A draft youth-specific AI policy exists but has not been adopted' },
      { v: 3, label: 'An adopted, HR- and legal-reviewed youth-specific AI policy is in place' }
    ]},
  { id: 'y3', fn: 'govern', module: 'youth', dimension: 'controls-evidence', depths: ['standard','comprehensive'],
    text: 'Does your organization maintain parental consent workflows for AI tools that interact with youth?',
    options: [
      { v: 0, label: 'No consent process' },
      { v: 1, label: 'General consent implicitly covers AI use' },
  ...
  { id: 'y4', fn: 'manage', module: 'youth', dimension: 'monitoring-response', depths: ['standard','comprehensive'],
    text: 'Do staff know the appropriate response if AI surfaces content indicating a youth safety concern?',
    hint: 'For example, signs of abuse, self-harm, or crisis identified through AI-analyzed communications or content.',
    options: [
      { v: 0, label: 'No protocol exists' },
      { v: 1, label: 'Addressed only through general mandated-reporter training' },
      { v: 2, label: 'An AI-specific protocol has been drafted, but staff have not been trained on it' },
      { v: 3, label: 'An AI-specific crisis protocol is in place, and staff have been trained accordingly' }
    ]},
  { id: 'y5', fn: 'map', module: 'youth', dimension: 'risk-classification', depths: ['comprehensive'],
    text: 'For AI tools accessible to youth on organizational premises, has age-appropriateness been reviewed?',
    hint: 'Common Sense Media publishes AI tool risk assessments for teen users; certain tools have documented safety failures involving minors.',
    options: [
      { v: 0, label: 'No review conducted' },
      { v: 1, label: 'Informal awareness only' },
      { v: 2, label: 'Reviewed on one occasion, with no process for subsequent updates' },
  ...
  { id: 'y6', fn: 'measure', module: 'youth', dimension: 'monitoring-response', depths: ['comprehensive'],
    text: 'Where AI is used in any youth-facing capacity, is that use tracked and subject to review?',
    options: [
      { v: 0, label: 'Not tracked' },
      { v: 1, label: 'Ad hoc awareness' },
      { v: 2, label: 'Logged, with no formal review process' },
...
export const REC_TITLES = {
  g1: 'Establish a foundational AI use policy',
  g2: 'Name an accountable owner for AI risk',
  g3: 'Implement an AI tool approval process',
  g4: 'Train employees on responsible AI use',
  g5: 'Establish AI risk as a standing leadership agenda item',
  m1: 'Establish an inventory of AI tools in use',
  ...
  m5: 'Incorporate AI-specific questions into vendor reviews',
  me1: 'Implement output quality checks',
  me2: 'Establish an AI incident log',
  me3: 'Implement bias checks for high-impact outputs',
  me4: 'Maintain records of AI decisions that affect individuals',
  me5: 'Schedule regular tool re-evaluation',
  ma1: 'Document an AI incident response plan',
  ma2: 'Ensure the ability to disable or roll back AI tools',
  ma3: 'Disclose when customers are interacting with AI',
  ma4: 'Establish an appeal path for AI-driven decisions',
  ma5: 'Trigger risk-posture reviews after significant changes',
  g6: "Set risk-based review levels for AI tools",
  g7: "Establish an AI tool decommissioning process",
  g8: "Establish a cross-functional AI adoption review",
  g9: "Define human-review vs. autonomous AI use per tool",
  g10: "Establish a protected channel for AI-risk reporting",
  g11: "Establish an external channel for reporting AI-driven issues",
  g12: "Route external feedback into policy decisions",
  g13: "Establish a vendor AI failure contingency plan",
  m6: "Consult end users before adopting AI tools",
  m7: "Document a mission-linked rationale for AI use",
  m8: "Re-evaluate whether AI tools deliver expected value",
  m9: "Document your organization's AI risk tolerance",
  m10: "Document the type of AI each tool employs",
  ...
  me7: "Implement independent review of AI tool performance",
  ...
  ma8: "Establish ongoing monitoring for deployed AI tools",
  // Nonprofit
  ...
  np5: "Audit the organization's CRM for AI features and license status",
  // Youth
  y1: 'Establish COPPA-compliant AI data handling',
  y2: 'Adopt a youth-specific AI policy',
  y3: 'Establish parental consent workflows for AI',
  y4: 'Establish an AI-surfaced youth safety protocol',
...
export const REC_BODIES = {
  ...
  g1: "A written AI-use policy is the baseline artifact that regulators, insurers, and customers expect to see first. It should be concrete, specifying which tools are approved, what data may not be entered, and who to consult regarding a new tool, and should be reviewed at least annually as the tool landscape changes.",
  g2: "Naming a specific, documented owner closes the accountability gap this project's research identified as most common: a majority of organizations report internal conflict over ownership of AI governance, and most individuals assigned the responsibility receive it without the authority required to act on it. A named owner with documented scope converts a general expectation that someone should address the issue into an accountable, executable responsibility.",
  g3: "A lightweight approval step prior to a new AI tool entering use, even a one-page checklist, identifies the highest-risk adoptions while they remain straightforward to redirect or decline, rather than after they have already become embedded in a workflow.",
  g4: "A one-time mention during onboarding does not remain effective as tools and risks evolve. Structured, role-specific training, updated as new tools and failure modes emerge, is what materially changes day-to-day behavior, as distinct from a record that AI was discussed on a single occasion.",
  g5: "AI risk warrants the same review cadence as other enterprise risks already tracked by leadership. Dedicated agenda time, even on a quarterly basis, keeps governance proactive rather than reactive, addressed only after an adverse event has occurred.",
  m1: "An organization cannot govern what it cannot see. A current, reviewed inventory of every AI tool in use, including AI features embedded within software already licensed by the organization, the most common blind spot, is the foundation on which every other governance activity in this report depends.",
  m2: "Identifying who could be harmed by an error, per use case, converts an abstract discussion of AI risk into something specific enough to act on, and is the direct input on which this report's risk-tiering and oversight recommendations depend.",
  m3: "Classifying use cases by risk tier, for example, low risk for drafting and high risk for hiring or credit decisions, allows oversight to be concentrated where it matters most, rather than applying the same shallow review uniformly across every tool.",
  m4: "Understanding what data an AI tool can access is a prerequisite for every downstream data-protection and vendor-risk decision. A tool with broad, unreviewed data access represents a materially greater exposure than its everyday use might suggest.",
  m5: "Vendor security questionnaires that omit AI-specific questions, such as training-data use, model update cadence, and human-review guarantees, fail to capture the risks unique to AI tools. A structured review incorporating those questions closes a genuine and common gap.",
  me1: "Spot-checking only when an output appears incorrect identifies the obvious failures but not the subtler ones. A structured review process supported by documented metrics provides an actual, ongoing signal on accuracy, rather than a general impression.",
  me2: "An incident log, even an informal one, converts a general sense that issues have occurred into a pattern that can be actively managed. Without tracking, the same failure mode can recur multiple times before it is recognized as a pattern.",
  me3: "Bias in AI outputs that affect individuals, including hiring, pricing, and customer service, carries genuine legal exposure under regimes such as NYC Local Law 144 and the Illinois AI Video Interview Act. Regular, structured testing, rather than informal spot checks alone, is what constitutes a defensible process.",
  me4: "Systematic logging of AI decisions that affect customers or employees, governed by a defined retention policy, constitutes the evidence base an organization would require if a decision were ever challenged. Retaining records only in response to a complaint leaves no evidence for the cases in which no complaint was raised.",
  me5: "Tools and their risk profiles change following adoption, as a vendor updates its model, a new regulation applies, or usage expands into new contexts. Re-evaluating on a defined cadence, rather than only once at adoption, keeps the organization's understanding of a tool current.",
  ma1: "A documented, tested incident response plan is the difference between a fast, controlled response and an improvised one when an incident first occurs. Testing it at least once confirms that it functions effectively under pressure, rather than only in principle.",
  ma2: "If disabling or rolling back an AI tool would require several days, that represents a genuine operational risk concealed behind otherwise normal day-to-day use. A documented, expedited rollback process limits the damage a faulty AI output can cause before it is stopped.",
  ma3: "Disclosing AI involvement by default, as a matter of policy, is both an emerging legal requirement in certain jurisdictions and a straightforward means of building trust. Inconsistent, case-by-case disclosure tends to surface as a problem only once its absence is noticed.",
  ma4: "A documented appeal path provides an individual affected by an AI-driven decision with a genuine means of contesting it. General customer support does not serve the same function, and its absence is a common driver of the 'black box' complaint that AI governance frameworks are designed to address.",
  ma5: "Standard practice is to review the organization's AI risk posture following every significant change, including a new tool, a new use case, a new regulation, or a major incident, rather than waiting until an adverse event has already occurred.",
  // Base -- R8's 33 GOVERN/MAP/MEASURE/MANAGE gap-fill items (B9)
  g6: "A documented risk-tolerance policy that determines review intensity, rather than applying the same level of scrutiny to every tool, allows limited governance capacity to be concentrated on the AI use cases that carry the most risk, consistent with NIST's GOVERN 1.3 framing of risk-based resourcing.",
  g7: "AI tools accumulate access and data over time, even after active use has ceased. A documented offboarding process, encompassing access revocation, disposition of the data the tool held, and a recorded rationale for retirement, closes a gap that is easily overlooked because no immediate consequence follows from omitting it.",
  g8: "NIST's framing addresses diversity of discipline and role, not solely demographics: who is genuinely consulted prior to an AI adoption decision, as distinct from who provides sign-off. A defined cross-functional review, encompassing legal, operations, the affected department, and IT/security, identifies risks that a single decision-maker is likely to overlook.",
  g9: "A general expectation to always verify AI output does not specify what any individual is responsible for. Documenting, per AI use case, what is human-reviewed as distinct from what operates autonomously renders accountability enforceable rather than aspirational.",
  g10: "Employees are often the first to notice an AI tool being misused or producing a risky output, though only if they know where to raise the concern and trust that doing so will not be held against them. A documented channel, as distinct from an informal expectation to mention it to a manager, is what actually enables that reporting to occur.",
  g11: "Customers and members of the public interacting with an organization's AI-driven systems require a genuine means of flagging a problem. General customer support that does not route AI-specific issues to any review process effectively provides no channel at all, even where one appears to exist.",
  g12: "A reporting channel is meaningful only if it produces a downstream outcome. Whether validated external feedback results in a documented policy or configuration change is what distinguishes a genuine feedback loop from a suggestion mechanism that goes unreviewed.",
  g13: "Vendor AI tools can fail, be breached, or prove to be seriously flawed, and evaluating a vendor prior to onboarding does not address what happens once the organization is already dependent on them. A documented contingency plan, including an assessment of how quickly the organization could operate without the tool, prevents a vendor failure from becoming the organization's own emergency.",
  m6: "Deploying a tool without consulting the intended day-to-day users is a common source of both poor adoption and unanticipated risk. Structured input from intended users, gathered prior to the decision and genuinely factored into it, surfaces problems that a top-down rollout would otherwise miss.",
  m7: "AI adoption without a stated purpose tends to expand tool by tool, with no shared standard for evaluating whether any of it is effective. A documented statement connecting AI use to the organization's mission or goals gives every other governance decision, including what to adopt and what to retire, a concrete basis for evaluation.",
  m8: "Adopting a tool without subsequently verifying whether it delivered the anticipated value is how organizations end up paying for AI that does not justify its cost, a consideration distinct from whether its outputs are accurate. Re-evaluating business value on a regular cadence, rather than only once, keeps that assumption grounded in evidence.",
  m9: "An explicit, written risk-tolerance statement, for example, that AI will not be used for a final hiring decision without human sign-off, converts leadership's informal views on acceptable AI risk into something that genuinely guides day-to-day decisions, and serves as the input on which this tool's own risk-tiering depends.",
  m10: "Determining what type of AI a tool employs, whether it generates content, classifies or scores individuals, or produces recommendations, is material because a scoring or classification tool carries meaningfully different risk than one that drafts text. Documenting this per tool is also the first input this tool's own use-case risk tiering requires.",
  m11: "Presuming an AI tool to be generally reliable, without documenting the specific situations in which its output should not be trusted unchecked, leaves individuals to determine this on a case-by-case basis. Documented known limitations per tool, rather than a general awareness that AI can produce incorrect output, is what genuinely informs when human review matters most.",
  m12: "Adopting a tool on the basis of perceived usefulness, without documenting the specific benefit expected, makes it impossible to later determine whether it delivered. Documenting the expected benefit, and the criteria for determining success, at the point of adoption is what makes a subsequent value re-evaluation possible at all.",
  m13: "Considering only the upside of an AI tool while omitting the downside, including cost, risk, and what could go wrong, is an easy blind spot when a tool is free or informally adopted. A documented cost/risk assessment as part of every adoption decision, not only major purchases, closes that gap.",
  m14: "Without a written boundary on the approved uses of an AI tool, use tends to expand informally into contexts that were never formally authorized, for example, a tool approved for internal drafting subsequently being used for external communications without review. A documented scope per tool keeps actual use aligned with what was reviewed.",
  m15: "General AI-use training does not teach a specific tool's known limitations, failure modes, or how to obtain reliable results from it. Tool-specific guidance, provided before individuals are expected to use a tool independently, is what genuinely builds operational competence, as distinct from organization-wide awareness training.",
  m16: "A general expectation that AI output should be reviewed is not equivalent to a defined checkpoint within the actual workflow. A documented human-review point for every tool, calibrated to its risk level, converts oversight from an assumption into a genuine step in the process.",
  m17: "For AI tools composed of multiple components, such as an internal workflow, a vendor's model, and a plugin or integration, assuming the vendor addresses security without verification is a common and easily overlooked gap. Documenting which risk controls fall under vendor responsibility and which fall under the organization's own clarifies where genuine exposure exists.",
  m18: "A one-time review at adoption does not indicate whether a tool continues to have the anticipated impact months or years later. A scheduled, recurring internal review, distinct from a reactive response to external complaints, identifies drift before it becomes a more significant problem.",
  me6: "Assessing whatever is easiest to measure, rather than what carries the most risk, creates a false sense of oversight. Defining specific metrics that begin with a tool's most significant risks, rather than its most convenient ones, is what NIST's MEASURE 1.1 requires and what makes measurement genuinely useful.",
  me7: "When the individual who implemented or advocates for a tool is also the one verifying whether it is operating safely, that does not constitute independent review; it represents a conflict of interest, even an unintentional one. At small-to-mid-size business scale, independence simply requires an individual not involved in the adoption decision, reviewing on a defined schedule.",
  me8: "Verifying a tool's safety once at setup, and not subsequently, leaves the organization unable to observe how its risk profile changes as usage grows or the underlying model is updated. Regular, scheduled safety checks, as distinct from general accuracy checks alone, are what genuinely identify a tool's potential for harm before it materializes.",
  me9: "This represents an active, current risk category, not a hypothetical one; 'prompt injection' is the industry term. A practical test drawn from current security guidance (OWASP, 2026): a tool with sensitive-data access, exposure to outside content, and the ability to take action externally should require human approval prior to acting; a tool meeting only one or two of those criteria presents lower risk. Assessing tools against this criterion, rather than presuming them safe, closes a genuine and current gap.",
  me10: "Lower priority than most other measurement gaps for a small-to-mid-size business deploying third-party tools rather than training its own models, though a growing number of vendors publish environmental-impact data, and certain clients and funders have begun to request it. Factoring this into vendor selection, even at a basic level, is a low-cost means of being prepared for that question.",
  me11: "Not every AI risk lends itself to a clean metric, for example, subtle bias in tone or nuanced judgment calls. Presuming those risks require no tracking simply because they are difficult to measure represents a gap; a defined approach, even periodic spot checks or structured review, is preferable to tracking only what is convenient.",
  me12: "Measurement designed without input from individuals who understand the specific use case tends to assess the wrong criteria effectively. Involving relevant expertise in determining what is measured, prior to obtaining results, is distinct from validating results afterward, and both are material.",
  me13: "Figures that stand without validation, absent confirmation from an individual with relevant context that they reflect reality, can be quietly misleading. Routinely reviewing measurement results with individuals who possess that context is what validates whether a tool is genuinely performing as intended, rather than merely producing figures that appear satisfactory.",
  me14: "A general impression that a tool appears to be performing adequately is not equivalent to the ability to identify specific evidence of improvement or decline over time. Tracking performance with comparable data points is what makes any subsequent re-evaluation more than an informal judgment.",
  ma6: "Allocating the same level of attention, or none, to every AI risk misallocates limited governance resources. NIST's Playbook guidance is specific on this point: resourcing and oversight attention should be explicitly matched to documented risk level, and that allocation should be reviewed periodically rather than assumed once.",
  ma7: "Every mitigation leaves some residual risk, and silently absorbing that risk differs materially from documenting it. NIST's Playbook is specific on this point as well: residual risk should be recorded as accepted, transferred, or minimally mitigated, and disclosed to whoever downstream is genuinely affected by it, rather than understood only informally by the individual who implemented the tool.",
  ma8: "A tool that continues to operate exactly as originally configured, with no active monitoring, access controls, or feedback mechanism, tends to drift from safe operation as its use evolves. This is distinct from a scheduled review; it concerns whether day-to-day technical and operational mechanisms genuinely exist to sustain safe performance between such reviews.",
  np1: 'Personal-account AI tools cannot be audited, revoked, or monitored by an organization. Nonprofit-tier enterprise accounts from Google (at no cost for up to 2,000 users), Canva (at no cost for up to 50 users), or Microsoft (Copilot Chat at no cost with an existing Microsoft 365 subscription) close this gap at little to no cost. This is often the highest-return single action a nonprofit organization can take.',
  ...
  np5: "CRM platforms such as Salesforce (Einstein), Bloomerang, and Raiser's Edge now include AI features that may be enabled by default or available as add-ons. A license audit determines what access the organization already has and prevents duplicate spend on external AI tools.",
  ...
  y4: 'Mandated-reporter obligations extend to signs of harm surfaced through AI. Staff require a specific protocol addressing the appropriate response when an AI tool flags content suggesting abuse, self-harm, or crisis, including escalation procedures and preservation of the AI-generated evidence.',
  y5: 'Common Sense Media and other independent bodies publish AI risk assessments specifically for teen users. Several mainstream AI tools have documented safety failures with minors. Any AI tool accessible to youth on organizational premises should be reviewed against these assessments prior to use.',
  y6: 'AI use in youth-facing capacities requires logging beyond general AI tool tracking, as the applicable review standard is higher. A designated staff role (e.g., AI Safety Lead or Youth Services Director) should periodically review what AI was used with youth, for what purpose, and whether that use aligned with policy.'
};
...
// ============================================================================
// B17, standard #6 ("methodology is documented well enough to be challenged"
// -- backlog SS1.4.9): synthesizes ALREADY-established, already-vetted
// research findings (R2/R3/R7/R9, all "done" research items with their full
// writeups elsewhere in the backlog) into report-facing reference content.
// Nothing here is a new claim -- Path A discipline (backlog SS1.6 rule 1)
// applies to external facts, and every fact below is one this repo already
// recorded as verified, not one being asserted fresh. Grounded specifically
// in: R2 (backlog SS1.4.5, the 5-dimension weighting + gating rationale),
// R3 (SS1.4.6, the four-tier risk model's basis), SS1.4.10 (why NIST AI RMF
// is the spine), R7 (Research list entry, the Big 4 framework comparison),
// and R9 (SS1.4.12, the functional-visibility taxonomy's actual evidence
// base). Rendered as a collapsible block in the report (src/ui.js), not a
// numbered section -- adding a 10th section would contradict B14's own
// "exactly nine sections" framing.
// ============================================================================
export const METHODOLOGY_NOTES = [
  { id: 'five-dimensions', title: 'Why five weighted dimensions, and why this weighting',
    body: 'Governance Maturity is a weighted average of five sub-dimensions (Ownership & Accountability 20%, Inventory & Visibility 20%, Risk Classification 20%, Controls & Evidence 25%, Monitoring & Response 15%), not a flat percentage of every question answered. The five-dimension taxonomy is independently corroborated by Superblocks\' own "five AI governance failure modes," which map one-to-one onto it. Controls & Evidence carries the highest weight because it is what an auditor tests, a conclusion backed by two independent sources: ISO 42001 audit-practice reporting and Superblocks\' documented "governance theater" failure mode.' },
  { id: 'gating-rule', title: 'Why a bottom-tier Ownership & Accountability score caps the overall score',
    body: 'A CSA whitepaper found that 73% of organizations report internal conflict over who owns AI governance, and 96% of the people assigned that responsibility receive it without real authority to act. That evidence shows Ownership & Accountability functioning as a structural prerequisite on which the other four dimensions depend, not merely one-fifth of a flat average. Accordingly, a bottom-tier score in that dimension caps the overall Governance Maturity score regardless of how the other four dimensions score, and independently qualifies as this tool\'s one critical-severity, dimension-level finding.' },
  { id: 'four-tier-risk-model', title: 'Basis for the four-tier per-use-case risk model',
    body: 'The New AI Tool Adoption assessment\'s four-tier structure and seven classification criteria were already sector-agnostic and did not require adaptation. The original Tier 1/2 trigger ("regulatory capital") was a banking-only concept that does not survive leaving financial services, so this tool uses the EU AI Act\'s Annex III high-risk domains instead. Oversight expectations are determined by a lookup of tier by company size, rather than applied as one universal column, crossed against a size-band table (1-10 owner-led informal, 11-50 executive review, 51-200 formal board plus part-time compliance role, 201-1000 dedicated governance role plus board committee, 1000+ full governance function). An independently built SMB framework found that only 1.5% of surveyed organizations believe they have adequate governance headcount, and addresses this by distributing ownership across existing roles rather than establishing new committees, which is why Tier 2 and Tier 3 oversight expectations are merged for organizations with fewer than approximately 200 employees. This is a structural design choice grounded in that framework, not a claim about what share of real-world AI use cases actually falls into each tier; no measured prevalence data supports a tier-distribution claim, and this tool does not assert one.' },
  { id: 'why-nist', title: 'Why NIST AI RMF is the spine, not ISO 42001 or the EU AI Act alone',
    body: 'NIST AI RMF is free, is the US reference standard, is sector-agnostic (it applies across every industry this tool covers without licensing), and crosswalks to both ISO 42001 and the EU AI Act; it is also the base that Big 4 consulting practices use. The alternatives each carry a disqualifying limitation for a tool of this kind: ISO 42001 is paywalled and certification-oriented, the EU AI Act is jurisdiction-specific, and Big 4 proprietary frameworks are not licensable for a tool built outside those firms.' },
  { id: 'differentiation', title: 'How this compares to published Big 4 frameworks',
    body: 'A direct review of the published AI governance materials of Deloitte, KPMG, EY, and PwC identified material structural differences worth noting: Deloitte operates two distinct structures under one brand, KPMG\'s ten-pillar framework is the most consistently documented, EY\'s approach is fragmented across several numbered structures, and PwC\'s "maturity staging" functions as a survey-segmentation lens rather than a prescriptive model with defined advancement criteria. The most consequential finding is that none of the four firms, including Deloitte, publish anything resembling this tool\'s weighted, gated, and fully documented scoring methodology. This is a substantiated, sourced differentiation point, not a marketing claim.' },
  { id: 'visibility-taxonomy', title: 'The functional-visibility taxonomy is a project-specific construct',
    body: 'The four-tag taxonomy that routes item-bank questions by vantage point (strategic, operational, technical-build, legal-compliance) was validated against AI-governance-specific organizational-design sources (Elevate Consult\'s AI Governance Operating Model & RACI framework, Knostic.ai\'s AI Governance Roles and Responsibilities primer). It is explicitly NOT supported by, and should never be cited alongside, the academic "Information Visibility Scale" (ter Hoeven, Stohl, Leonardi & Stohl, 2021, Management Communication Quarterly); that instrument measures a materially different construct (Availability, Approval, and Accessibility of general information visibility) and does not distinguish strategic, operational, technical, or compliance levels at all. This taxonomy is this project\'s own organizational-design construct, not an application of published academic research.' }
];
